...
void loop() {
  Serial.printf("##################################################################################\n");
  Serial.printf("Welcome to the curve confiurator! Type in the number corrsponding to an option below and hit enter.\n");
  ...
  else if(input == '2') // Set curve configuration
  {
    ...
    else if(input == '2') // Write command w/linear data
    {
      ...
      while(Serial.available()) {Serial.read();} // Flush the serial RX buffer
